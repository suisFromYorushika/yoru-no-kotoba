...
- 乐队在网页里的顺序（筛选、专辑架）跟 `albums` 里第一张专辑出现的先后一致，不用改代码。
...
- 专辑的 `short` 是简称，只给很长的名字（例：音辞、夏草）；网页的标签、例句出处等地方显示简称，专辑页和提示里显示全名。`cover` 是 `web/covers/` 里的封面（专辑架上显示它；没有封面就显示名字）。`"badge": false` 表示这张不是真正的专辑（例：单曲与合作曲），词的出处标签里不显示它。
...
## data/readings.json（读音和原形修正）

UniDic 有些读音在歌词里不对（何も 读成 なんも、君 读成 くん、明日 读成 あす…）。网页会在汉字上显示这些读音，所以分词时按这张表修正 `k`：

```jsonc
{"tok": {"s": "何", "next": {"s": "[もかがを]"}}, "k": "なに", "note": "何も/何か/何が/何を 读 なに"}
```

改了 `k` 以后，原形的读音 `lk` 会跟着改（開い 改成 あい → 原形 開く 读 あく），弹窗里的「原形」就不会还是 ひらく。

UniDic 还会把普通的词认错（あ、そうか 当成人名 爽果；金が足りない 的 金 当成人名 キン；覚束ぬ 拆成人名 覚 + 束）。这种用 `l`（原形）、`lk`、`p` / `p2`（词性）改：

```jsonc
{"line": "あ、そうか", "tok": {"s": "そうか"}, "l": "そうか", "p": "感動詞", "p2": "一般"}
```

改原形会影响词表计数（词表按原形匹配），所以改完要跑 `make analyze export`，看一下哪些词的次数变了。

`tok` 的写法和 match 条件相同（可以用 prev/next）；加 `"line": 正则` 表示只在这一行日文包含它时才改（例：`溜息を吐く` 的 吐く 读 つく，`息を吐く` 读 はく）。同一个词有几条都满足时只用第一条。整理新专辑时用 `make audit` 看到可疑读音，就往这里加一条。

另外，人名地名（固有名詞）的原形 UniDic 写成片假名（小平 → コダイラ），分词时自动改回原文写法。

## data/names.json（人名、地名注释）

`{原形: 注释}`，例如 `"ニーチェ": "尼采（…），德国哲学家。"`。点歌词里的名字（或者词表里的 エルマ 这类词）时，词卡里会显示「是谁 / 在哪」。键是原形，被拆成几段的名字（キ + ルナ）先在 readings.json 里把原形改成整个名字（キルナ）。

## data/data.json（网页用，自动生成）

- `albums` / `pending`：已整理 / 待整理的专辑，含简称 `short`（例：音辞）和封面 `cover`
- `names`：data/names.json 原样导出
