...
网址：https://suisfromyorushika.github.io/yoru-no-kotoba/ （main 分支更新后自动发布）。进度保存在本机浏览器，登录后同步到云端（Supabase，见 [docs/supabase.md](docs/supabase.md)）。

以前发布过的 claude.ai 版已不再更新。
...
- **关系图**：以一个词为中心，画出和它关系最近的 14 个词，关系可以选「同一句里一起出现」「同一首歌里一起出现」「写法里有同一个汉字」。点周围的词就换成它做中心，下面列出一起出现的原句。词卡里也有「关系图」按钮。
- **词表 / 语法**：按出现在几首歌里排序，展开能看歌词例句和全部原句（先显示 4 首歌、每首 3 句，其余点按钮展开，底部可以收起）。搜索框支持汉字、假名、罗马音和中文，也会列出包含这个词的歌词。
- **歌曲**：顶部推荐「下一首听什么」（按你已认识的词算覆盖率）；下面是专辑架（按乐队分组、一行三张封面，下面写专辑名，底边是已掌握的比例），点封面进专辑；每首歌有 QQ 音乐 / 网易云 / Spotify / Apple Music / YouTube 链接和完整歌词对照。
- **播放器**：在「我的 → 播放器」里填上自己部署的网易云 API 服务器（家里 NAS 上的 api-enhanced，见 [docs/player.md](docs/player.md)）和访问密码，登录网易云会员后就能在歌曲页直接播放：底部播放条显示正在唱的那句（带注音和中文，点词查意思），歌词对照里当前句高亮、点「▶ 00:46」从这句开始，还有「再听这句」「下一句」「单句循环」。切到闪卡、词表时音乐不停。这些设置和网易云登录凭证只存在本机（`yoru_player_v1`），不进云同步。
- 所有歌词都在汉字上注假名、按词分段、每段下标罗马音（「我的」里可以改成按字对齐：每个汉字、每个假名下面各标各的；按词时还可以打开「用 · 分开词和词尾」，例如 wasure·rare·nai），附中文翻译和 LRC 时间；**点任意一个词**会弹出词卡（词表词显示意思，助词助动词显示用法说明，还有原形、词性和相关语法点）。
- 词、例句和每行歌词都有朗读按钮（用设备自带的日语语音，不需要联网服务）。
- 「我的」面板：账号同步状态和昵称（显示在右上角，存在 Supabase 账号资料里）、学习记录（今天复习数、新掌握、连续天数、最近 14 天）、显示和朗读设置、进度备份。
- 进度保存在本机 `localStorage`（`yorushika_learned_v1` 已掌握、`yoru_srs_v1` 复习记录、`yoru_log_v1` 每日记录），登录后存到 Supabase。「我的 → 备份进度」可以在设备或浏览器之间搬运进度。
...
data/library.json      本地歌词库的乐队 / 专辑文件夹 → 乐队、专辑 id 的对照表
...
data/site.json         网址和 Supabase 配置
...
make ingest    # 从 ~/Documents/Gemini Spark/Lyrics 导入新歌（可用 LYRICS=路径 指定）；新歌手、新专辑先在 data/library.json 里加一行
